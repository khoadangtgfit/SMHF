...
public class SmartHFManagerFrm extends JFrame {
	 ...
	 public SmartHFManagerFrm(boolean inReadOnly) {
        ...
        mPanel = new SmartHFManagerPnl(isReadOnly);
        init();
	 ...
	 private void init(){
		 LOGGER.info("Start.");
	        add(getPanel());
	        try {
	            setIconImage(ImageIO.read(new File(IconKey.LBRS_ICON)));
	        } catch (IOException e) {
	            LOGGER.error("Fail to load icon");
	            e.printStackTrace();
	        }
	        ...
	        setTitle(SmartHFResource.getResource(LangKey.WORD_SMARTHFTITLE));
	 ...
	 private void confirmExit(){
		 int confirmed = JOptionPane.showConfirmDialog(this,
             SmartHFResource.getResource(LangKey.MSG_INF_EXITCONFIRM), 
